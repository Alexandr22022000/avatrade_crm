module.exports = {
    ...
    GET_SELLS: 'SELECT sales.id AS date, stores.name AS store, sales.store_id, users.name as manager, sales.user_id as manager_id, sales.price, sales.services FROM sales JOIN stores ON sales.store_id = stores.id JOIN users ON sales.user_id = users.id WHERE (sales.id BETWEEN $1 AND $2) AND ((sales.user_id = $3) OR $3 IS NULL) AND ((sales.store_id = $4) OR $4 IS NULL) ORDER BY sales.id DESC;',
