...
module.exports = (start, end) => {
    return new Promise((resolve, reject) => {
        ...
        query('SELECT SUM(value) AS value, store_id FROM cashbox_collection WHERE id < $1 GROUP BY store_id', [start])
            .then(callback(null, ({rows}) => {
                data.oldCollections = rows;
            }));

        query('SELECT users.id, users.name, users.status, ranks.payment FROM users JOIN ranks ON users.rank = ranks.id WHERE users.id < $1 ', [end])
            .then(callback(null, ({rows}) => {
                data.users = rows.filter(item => item.status === 0);
                data.inactiveUsers = rows.filter(item => item.status === 1);
            }));

        query('SELECT id, name FROM stores WHERE status = 0 AND id < $1', [end])
            .then(callback(null, ({rows}) => {
                data.stores = rows;
            }));

        query('SELECT workdays.id, workdays.user_id, workdays.calendar, workdays.values, workdays.descriptions, users.name FROM workdays JOIN users ON workdays.user_id = users.id WHERE workdays.date >= $1 AND workdays.date < $2', [start, end])
            .then(callback(null, ({rows}) => {
        ...
        query('SELECT payments.id, payments.user_id, payments.value, users.name FROM payments JOIN users ON payments.user_id = users.id WHERE payments.date >= $1 AND payments.date < $2', [start, end])
            .then(callback(null, ({rows}) => {
                data.payments = rows;
            }));
    });
